...
function getLevel(levels){ //calls Level() based on what level you are on. CALLED IN: mySprite update
	switch(levels){
		case 1:
			Level(1); //speed:1 & wave:3
			break;
		case 2:
			Level(2); //speed:2 & wave:6
			break;
		case 3:
			Level(2); //speed:2 & wave 12
			break;
		case 4:
			Level(3); //speed:3 & wave: 6
			break;
		case 5:
			Level(3); //speed:3 & wave:12
			break;
		case 6:
			Level(4); //speed:4 & wave:6
			break;
		default:
...
function Level(s){ //speed, wave, parts[] CALLED IN: Init
	//for(1 <= level <= 3){
	//Particle.speed = s;
...
function callWave(wave, levels){ //CALLED IN: Particle Update
	switch(levels){
		case 1:
			switch(wave){
				case 3:		
...
/** callLevel() for ongoing waves **/
function callLevel(wave, levels){ //CALLED IN: Particle update (instead of callWave())
	switch(levels){
		case 1:
			switch(wave){
				case 3:
		...
		default:
			switch(wave){
				case 3:		
					Level(10);
					console.log("default of callLevel()");
					//console.log(levels);
					break;
				default:
					break;
...
/**Delete any prexisting particles on screen (for ongoing waves)**/
function deleteWave(){	
	for(var i = 0; i < parts.length; i++){
		world.removeChild(parts[i]);
	}
...
gameScreen.init = function() {
	...
	getLevel(levels); //comet movement is fine (as opposed to if called in sprite update)
	
	mySprite.update = function(d) {
		gameScreen.scrollY += .5;
		scoreDisplay.text = "Score: "+score;
		//level generation based on level number
		//getLevel(levels); //makes comet just fly off the screen, won't bounce
    	//console.log(score);
